ObserveRoll <- function(input,output){
  ...
  if(data2$level < 90){
    output$noob <- renderImage({list(src = "grindkek.gif", align = "center", alt = "B2TG")},
                               deleteFile = FALSE)
  ...
  if(input$slider!= 0){
    ...
    if(length(itemnames)==0){
      output$grinder <- renderImage({list(src = "grinder.jpg", alt = "FAG")},
                                    deleteFile = FALSE)
    ...
    showModal(modalDialog(
      fluidRow(column(1, align="center",offset = 3,
                      imageOutput("clown"), 
                      textOutput("cointxt"))),easyClose = TRUE
    ))
  }
  return(output, roll , BN2)
}